...
public class DocumentRead extends BaseController {
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/** 조회수 중복갱신 방지 처리 - 24시간 동안 중복 갱신되지 않는다. */
		String cookieKey = "document_"+category+"_"+documentId;
		String cookieVar = web.getCookie(cookieKey);
		
		try {
			if(cookieVar == null) {
				bbsDocuService.updateDocumentHit(document);
				web.setCookie(cookieKey, "Y", 60*60*24);
			}
			readDocument = bbsDocuService.selectDocument(document);
			prevDocument = bbsDocuService.selectPrevDocument(document);
			nextDocument = bbsDocuService.selectNextDocument(document);
